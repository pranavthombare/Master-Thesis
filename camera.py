    ...
    def image_preprocessing(self):
        ...
        if len(face)>0:
            ...
            if self.check == 1:
                ...
                self.stored_encoding = img_to_encoding("face.png",FRmodel)

                if self.check ==1 and self.inf is not None:
                    score = self.inf.verify("face.png",self.email,self.database,FRmodel)
                    print("Score:-  {}".format(score))
                    if score<=0.7:
                        messagebox.showinfo("Success","Person Verified is "+str(self.email))
                    else:
                        messagebox.showinfo("Failure","Person is not "+str(self.email))
                return True

            elif self.check == 0:
                cv2.imwrite("images/"+self.email+".png",resize_img)
                self.open_database()

                if self.database.get(self.email,"") == "":
                    from fr_utils import img_to_encoding
                    FRmodel = self.inf.returnModel()
                    self.database[self.email] = img_to_encoding("images/"+self.email+".png",FRmodel)
                ...
                self.database[self.email] = img_to_encoding("images/"+self.email+".png",FRmodel)
                self.close_database()
                messagebox.showinfo("Success","{} face is updated".format(self.email))
                return True
